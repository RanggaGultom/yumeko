...
export default {
    ...
    // COMMAND RUNNER
    COMMAND_RUNNER_ONLY_NSFW: () => "❌ **| Command iki mung bisa kaleksanan ing channel NSFW**",
    COMMAND_RUNNER_ONLY_ONE: user => `**❌ | ${user}, kowe ora iso nganggo command iki luwih saka 1**`,
    COMMAND_RUNNER_IN_COOLDOWN: (user, amount) => `**⏱️ | ${user}, sabar!.** kowe iso nganggo meneh ing \`${Math.round(amount)}\` detik`,
    COMMAND_RUNNER_MISSPERMS: (user, perm, isclient) => `**❌ | ${isclient ? "Kulo" : `${user}, kowe`} mbutuhake permission iki kanggo ngelakokake command. ${perm}**`,
    COMMAND_RUNNER_ERROR: stack => `**🚫 | Nyuwun sewu, pangembang bot iki rada kesel.** ${stack}`,

    // ARGUMENT PARSER
    ARGUMENT_PARSER_PROMPT: (sign, prompt) => stripIndents`
        **${sign} |** ${prompt}
        **▫️ |** *Kowe duwe \`30\` detik kanggo nemtokake*
        **▫️ | ** *Ketik \`cancel\` kanggo mbatalke.*
        **▫️ | ** *Utawa yen kowe pengin nulis cancel, gunakake \`|cancel|\`*
    `,
    ARGUMENT_PARSER_CANCELED: () => "👌 **| Dibatalke**",
    ARGUMENT_PARSER_NOT_UNDERSTAND: user => `**🤷 | ${user}, Kayane kowe ora ngerti tenan cara nggunakake command iki.**`,

    // TYPE
    TYPE_CODE_NOT_FOUND: () => "**Tulung lebokke codeblock. kayak:** ```kiye```",

    TYPE_COMMAND_HAS_SIMILIAR: (content, similiar) => `**\`${content}\` ora ono.** ${similiar.length ? ` ? ${similiar}` : ""}`,
